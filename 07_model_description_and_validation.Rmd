...
```{r echo=FALSE, opts.label='fig.large', fig.width=15}
plot_grid(
...
```{r echo=FALSE, opts.label='fig.wide', fig.width=15}
plot_grid(pl_tot, pl_exp, pl_res, 
          nrow = 1, rel_widths = c(1, 1.05, 1))
```
...
- $\color{purple}{h_{ii}}$ — "сила воздействия" отдельных наблюдений (leverage, рычаг)
...
```{r non-linear, echo=FALSE, purl=FALSE, fig.height=5, fig.width=15}
...
```{r homo-hetero, echo=FALSE, purl=FALSE, opts.label='fig.wide.taller', fig.width=15}
N <- 300
b_0 <- 0.5
b_1 <- 8
...
```{r hetero-log-transform, echo=FALSE, purl=FALSE, opts.label='fig.wide.taller', fig.width=15}
