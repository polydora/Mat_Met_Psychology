...
# Создаем последовательность значений x от -10 до 10
x = seq(-10, 10, length.out = 100)

# Задаем уравнение параболы y = x²
y <- x^2
...
# Добавляем легенду
legend("topleft", legend = "y = x²", col = "blue", lwd = 2)
...
box_weight <- 1.5
apples <- 6
box_weight / apples # ошибка. переменная box еще не была создана.
...
length(seq(from = -1, to = 1, by = 0.25))
...
num_starships <- c(2, 1, 5, 0)

Jedy <- data.frame(name = person_name,
                   homeworld = c('Tatooine', 'Tatooine', 'Stewjon', NA),
                   num_starships = num_starships)
